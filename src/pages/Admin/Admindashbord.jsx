    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          ...
          // 1. Obtener usuarios
          const usersResponse = await axiosClient.get("users/api/");
          const usersCount = extractCount(usersResponse.data);
          
          // 2. Obtener restaurantes
          const restaurantsResponse = await axiosClient.get("restaurante/api/");
          const restaurantsCount = extractCount(restaurantsResponse.data);
          
          // 3. Actualizar estado con los datos obtenidos
          setData({
            users: usersCount,
            restaurants: restaurantsCount,
            orders: 0, // Inicializar en 0 (implementar luego)
            revenue: 0, // Inicializar en 0 (implementar luego)
            loading: false,
            error: null,
            growthData: null, // Inicializar (implementar luego)
            topProducts: [], // Inicializar (implementar luego)
          });
      ...
      // Función auxiliar para extraer el conteo de diferentes estructuras de respuesta
      const extractCount = (responseData) => {
        if (Array.isArray(responseData)) {
          return responseData.length;
        }
        if (typeof responseData === 'object' && responseData !== null) {
          return responseData.count || responseData.total || responseData.length || 0;
        }
        return 0;
      };
